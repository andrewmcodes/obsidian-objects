...
- Formatting is handled by Prettier (`@andrewmcodes/prettier-config`): run
  `mise run format`. Indentation (2-space) and end-of-line rules come from
  `.editorconfig`.
...
- `tsconfig.json` targets ES2022 with `moduleResolution: "bundler"` and
  `noEmit` (esbuild does the bundling; `tsc` only type-checks).

See [linting.md](linting.md) for ESLint and [architecture.md](architecture.md)
