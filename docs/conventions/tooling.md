...
- Package manager: **pnpm via Corepack** (pinned by `packageManager` in
  `package.json`); run as `corepack pnpm …`.
- `mise run check` runs lint + format-check + build + test together — run it
  before opening a PR.
...
- Dependabot proposes weekly updates (grouped dev-dependencies) — see
