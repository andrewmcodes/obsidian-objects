...
Rule of thumb: **pure logic must not import from `obsidian`** so it stays
unit-testable in plain Node. Anything touching the vault, workspace, or UI lives
in a service method, modal, view, or command — never in the pure helpers.
